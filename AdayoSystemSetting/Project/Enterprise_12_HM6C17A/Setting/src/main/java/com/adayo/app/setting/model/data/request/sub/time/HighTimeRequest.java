...
public class HighTimeRequest extends BaseRequest {
    private final static String TAG = HighTimeRequest.class.getSimpleName();
    private final MutableLiveData<String> mTimeLiveData = new MutableLiveData<>();private final MutableLiveData<Boolean> mAutoTimeSyncLiveData = new MutableLiveData<>();private final MutableLiveData<Boolean> m24ClockModeLiveData = new MutableLiveData<>();private final Handler mHandler = new Handler();
    ...
    private void parseAutoTimeSync(String json) {
        ...
        boolean isTimeNetSync = (boolean) map.get(ParamConstant.SHARE_INFO_KEY_TIME_NET_SYNC);
        boolean isTimeGpsSync = (boolean) map.get(ParamConstant.SHARE_INFO_KEY_TIME_GPS_SYNC);
        mAutoTimeSyncLiveData.setValue(isTimeNetSync || isTimeGpsSync);}
    ...
    public void requestAutoTimeSync(boolean enable) {
        LogUtil.i(TAG, "enable = " + enable);
        if (enable) {
            SettingsSvcIfManager.getSettingsManager().setAutoSyncTime(1 | 2);} else {
            SettingsSvcIfManager.getSettingsManager().setAutoSyncTime(0);
        }
    }
...
public void unInit() {
    ...
    public MutableLiveData<String> getTimeLiveData() {
        if (mTimeLiveData.getValue() == null) {
    ...
    public MutableLiveData<Boolean> getAutoTimeSyncLiveData() {
        if (mAutoTimeSyncLiveData.getValue() == null) {
            mAutoTimeSyncLiveData.setValue(false);
            LogUtil.w(TAG, "mAutoTimeSyncLiveData is null");
        }
        return mAutoTimeSyncLiveData;
    }

    public MutableLiveData<Boolean> get24ClockModeLiveData() {
    ...
    private void run() {
        String format;
        LogUtil.i(TAG, "get24ClockModeLiveData =" + get24ClockModeLiveData().getValue());
