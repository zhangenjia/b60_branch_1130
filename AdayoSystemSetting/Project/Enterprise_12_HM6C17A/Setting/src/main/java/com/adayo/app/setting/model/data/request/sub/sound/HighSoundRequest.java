...
public class HighSoundRequest extends BaseRequest {
    private final static String TAG = HighSoundRequest.class.getSimpleName();
    ...
    private static final int AUDIO_STREAM_NAVI = 11;
    ...
    private final MutableLiveData<Map<Integer, int[]>> mEqLiveData = new MutableLiveData<>();private final MutableLiveData<int[]> mSoundFiledLiveData = new MutableLiveData<>();private final MutableLiveData<Integer> mDtsModeLiveData = new MutableLiveData<>();private final MutableLiveData<Integer> mDtsModeLastLiveData = new MutableLiveData<>();
    private final MutableLiveData<Boolean> mBeepEnableLiveData = new MutableLiveData<>();private final MutableLiveData<Integer> mPhoneRingLiveData = new MutableLiveData<>();private final MutableLiveData<Integer> mSpeedLevelLiveData = new MutableLiveData<>();private final MutableLiveData<Integer> mLoudnessMediaLiveData = new MutableLiveData<>();private final MutableLiveData<Integer> mLoudnessPhoneLiveData = new MutableLiveData<>();private final MutableLiveData<Integer> mLoudnessTtsLiveData = new MutableLiveData<>();
    private final MutableLiveData<Integer> mLoudnessNaviLiveData = new MutableLiveData<>();
    private final MutableLiveData<Integer> mLoudnessBtMusicLiveData = new MutableLiveData<>();private final MutableLiveData<Integer> mLoudnessBtPhoneRingLiveData = new MutableLiveData<>();
    ...
    public void parseLoudnessSwitch(String json) {
        Map<String, Object> map = FastJsonUtil.jsonToMap(json);
        if (map == null) {
            LogUtil.w(TAG, ".SHARE_INFO map is null");
            return;
        }
        Boolean b = (boolean) map.get(ParamConstant.SHARE_INFO_LOUDNESS_SWITCH);
        LogUtil.i(TAG, "b = " + b);
        if (b) {
            mLoudnessSwitch.setValue(true);
        } else {
            mLoudnessSwitch.setValue(false);
        }
    }
    ...
    public void parseLoudness(String json) {
        ...
        Integer ttsProgress = (Integer) jsonArray.get(AUDIO_STREAM_TTS);
        Integer naviProgress = (Integer) jsonArray.get(AUDIO_STREAM_NAVI);
        ...
        if (!Objects.equals(ttsProgress, getLoudnessTtsLiveData().getValue())) {
            mLoudnessTtsLiveData.setValue(ttsProgress);}
        if (!Objects.equals(naviProgress, getLoudnessNaviLiveData().getValue())) {
            mLoudnessNaviLiveData.setValue(naviProgress);}
        ...
        LogUtil.debugD(TAG, "mediaProgress =" + mediaProgress + "phoneProgress =" + phoneProgress + "ttsProgress =" + ttsProgress + "naviProgress =" + naviProgress + "btMusicProgress =" + btMusicProgress + "btPhoneRingProgress =" + btPhoneRingProgress);
    }
    ...
    public void requestLoudnessSwitch(boolean b) {
        LogUtil.i(TAG, "b =" + b);
        if (b) {
            SettingsSvcIfManager.getSettingsManager().setLoudnessSwitch(1);
        } else {
            SettingsSvcIfManager.getSettingsManager().setLoudnessSwitch(0);
        }
...
SettingsSvcIfManager.getSettingsManager().setSysBeepSwitch(enable ? 1 : 0);
    ...
    public void requestLoudness(int streamType, int volume) {
        LogUtil.i(TAG, "streamType = " + streamType + "volume = " + volume);
    ...
    public void init() {
        ...
        if (b) {
            ...
            parseLoudnessSwitch(eqAndEffectJson);
        } else {
            initRegisterEQ();
        }
    ...
    public LiveData<Integer> getLoudnessPhoneLiveData() {
        if (mLoudnessPhoneLiveData.getValue() == null) {
            ...
            LogUtil.w(TAG, " mLoudnessPhoneLiveData     is null");
        }
        return mLoudnessPhoneLiveData;
    }

    public LiveData<Integer> getLoudnessTtsLiveData() {
        if (mLoudnessTtsLiveData.getValue() == null) {
            mLoudnessTtsLiveData.setValue(0);
        }
        return mLoudnessTtsLiveData;
    }

    public LiveData<Integer> getLoudnessNaviLiveData() {
        if (mLoudnessNaviLiveData.getValue() == null) {
            mLoudnessNaviLiveData.setValue(0);
        }
        return mLoudnessNaviLiveData;
    }
    ...
    public MutableLiveData<Boolean> getLoudnessSwitch() {
        if (mLoudnessSwitch.getValue() == null) {
            mLoudnessSwitch.setValue(false);
        }
        return mLoudnessSwitch;
    }
}
