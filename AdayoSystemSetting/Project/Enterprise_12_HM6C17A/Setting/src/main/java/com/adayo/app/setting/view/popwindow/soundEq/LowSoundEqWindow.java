...
public class LowSoundEqWindow implements SeekBar.OnSeekBarChangeListener, ISoundEQ<SoundEqWindow, PopupSoundEqBinding> {
    private final static String TAG = LowSoundEqWindow.class.getSimpleName();
    ...
    public void initData() {
        mViewModel.mLowSoundRequest.getEqLiveData().observe(mActivity, new Observer<Map<Integer, int[]>>() {
            @Override
            public void onChanged(@Nullable Map<Integer, int[]> integerMap) {
               LogUtil.debugD(TAG, "Eq");
                if (isChange) {
