...
import com.adayo.app.setting.viewmodel.fragment.Sys.HighSysViewModel;
...
public class HighSysFragment implements IFragmentConfig<SysFragment, FragmentSysBinding> {
    private final static String TAG = HighSysFragment.class.getSimpleName();
    protected SysFragment fragment;
    private FragmentSysBinding mViewBinding;
    private static final int MULTIPLE_CLICKS = 5;private static final long MULTIPLE_CLICKS_DURATION = 2000;private long[] mMultipleHits = new long[MULTIPLE_CLICKS];private Badge mBadgeView;
    private HighSysViewModel mViewModel;
    ...
    public void bindData() {
       LogUtil.debugD(TAG, "");
        mViewModel = ViewModelProviders.of(fragment.requireActivity()).get(HighSysViewModel.class);
    ...
    public void initData() {
       LogUtil.debugD(TAG, "");
        mViewModel.mHighVerRequest.getHasUsbFileLiveData().observe(fragment.getViewLifecycleOwner(), new Observer<Boolean>() {
            @Override
            public void onChanged(@Nullable Boolean aBoolean) {
                mViewBinding.btnUpgradeUsb.setEnabled(aBoolean);
            }
        });
        mViewModel.mHighVerRequest.getVerLiveData().observe(fragment.getViewLifecycleOwner(), new Observer<String>() {
            @Override
        ...
        mViewModel.mHighVerRequest.getHasLatestLiveData().observe(fragment.getViewLifecycleOwner(), new Observer<Boolean>() {
            @Override
            public void onChanged(@Nullable Boolean aBoolean) {
               LogUtil.debugD(TAG, "ke geng xin ban ben biao ji = " + aBoolean);
                mBadgeView.setBadgeNumber(aBoolean ? 1 : 0);
            }
        });
...
changeSelected(mViewBinding.clUpgrade, false);
    ...
    public void initView() {
       LogUtil.debugD(TAG, "");
        mBadgeView = new QBadgeView(fragment.requireContext());mBadgeView.bindTarget(mViewBinding.tvBadge)
                .setBadgeGravity(Gravity.CENTER)
                .setBadgeTextSize(20, true);
    }
...
mViewBinding.tvSysCurrentVerTitle.setOnTouchListener((view, motionEvent) -> multipleClick());
        ...
        mViewBinding.btnUpgradeUsb.setOnClickListener(v -> {
           LogUtil.debugD(TAG, "btnUpgradeUsb");
            ...
            mViewModel.mHighVerRequest.requestLoUpgrade();
        });

        mViewBinding.btnUpgradeOta.setOnClickListener(v -> {
           LogUtil.debugD(TAG, "btnUpgradeOta");
            if(v.isPressed()){
                mSysHighlight.mSysHighlightRequest.requestHighlightModule(HIGHLIGHT_UPGRADE);
            }
            mViewModel.mHighVerRequest.requestOlUpgrade();
        });
    }
    ...
    private boolean multipleLogClick() {
        System.arraycopy(mMultipleHits, 1, mMultipleHits, 0, mMultipleHits.length - 1);mMultipleHits[mMultipleHits.length - 1] = SystemClock.uptimeMillis();if (mMultipleHits[0] >= (SystemClock.uptimeMillis() - MULTIPLE_CLICKS_DURATION)) {
            int x= Settings.Global.getInt(fragment.getActivity().getContentResolver(),"LogUtil_debug",0);
            Intent intent = new Intent("LogUtil_debug");
            if(x==1){
                intent.putExtra("LogUtil_debug",0);
            }else {
                intent.putExtra("LogUtil_debug",1);
            }
            intent.setPackage("com.adayo.app.setting");
            fragment.getActivity().sendBroadcast(intent);
            LogUtil.i(TAG,"change LogUtil");
