...
public class HighVerRequest extends BaseRequest {
    private final static String TAG = HighVerRequest.class.getSimpleName();
    private final MutableLiveData<String> mVerLiveData = new MutableLiveData<>();
    private final MutableLiveData<Boolean> mHasLatestLiveData = new MutableLiveData<>();
    ...
    public void requestCurrentVersion() {
        UpgradeManager.getInstance().getServiceConnection();
        String currentVersion = SystemProperties.get("persist.adayo.mpu.version");
    ...
    private final IUpgradeCallback.Stub mUpgradeCallback = new IUpgradeCallback.Stub() {
        ...
        public void notifyFotaDownloadProgress(int downloadProgress) throws RemoteException {
            LogUtil.debugD(TAG, "downloadProgress = " + downloadProgress);
        ...
        public void notifyNewVersionInfo(String latestVersionInfo) throws RemoteException {
            LogUtil.i(TAG, "latestVersionInfo = " + latestVersionInfo);
            Map<String, Object> map = FastJsonUtil.jsonToMap(latestVersionInfo);
            if (map == null) {
                LogUtil.w(TAG, ".SHARE_INFO map is null");
                return;
            }
            String updateType = String.valueOf(map.get("UpdateType"));
            String newVersion = String.valueOf(map.get("new_version"));
            ...
            } else if ("FOTA".equals(updateType)) {
                if (!isExistNewVersion(getVerLiveData().getValue(), newVersion)) {
                    mHasLatestLiveData.postValue(false);
                } else {
                    mHasLatestLiveData.postValue(true);
                }
                LogUtil.d(TAG,"mHasLatestLiveData ="+mHasLatestLiveData.getValue());
            }
        }
    };
    ...
    public void requestOlUpgrade() {
        LogUtil.d(TAG, "");
        UpgradeManager.getInstance().onUpgrade(1);
    }
    ...
    public LiveData<Boolean> getHasLatestLiveData() {
        if (mHasLatestLiveData.getValue() == null) {
            mHasLatestLiveData.setValue(false);
        }
        return mHasLatestLiveData;
    }
