...
public class LowHarmanActivity implements IHarmanViewOutData, IHarmanActivity<HarmanPopActivity, ActivityHarmanBinding> {
    private final static String TAG = LowHarmanActivity.class.getSimpleName();
    ...
    public void registerFragment(HarmanPopActivity activity, ActivityHarmanBinding viewbinding) {
        ...
        mViewModel = ViewModelProviders.of(mActivity).get(LowSoundViewModel.class);
        mIHarmanViewInData = mViewBinding.viewHarman;
        mIHarmanViewInData = mIHarmanViewInData.init(this);
        mIHarmanViewInData.initPostion(mViewModel.mLowSoundRequest.getSoundFiledLiveData().getValue()[0], mViewModel.mLowSoundRequest.getSoundFiledLiveData().getValue()[1]);
    ...
    public void initView() {
        LogUtil.debugD(TAG, "");
        mViewBinding.tvTitle.setText(mActivity.getResources().getString(R.string.sound_field_adjustment));
        mViewBinding.cbQls.setVisibility(View.GONE);
        FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);layoutParams.setMargins(219, 488, 0, 0);mViewBinding.btHarmanReset.setLayoutParams(layoutParams);}

    @Override
    public void initData() {
        LogUtil.debugD(TAG, "");
        mViewModel.mLowSoundRequest.getSoundFiledLiveData().observe(mActivity, new Observer<int[]>() {
            @Override
    ...
    public void initEvent() {
        ...
        mViewBinding.btHarmanReset.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                LogUtil.debugD(TAG, "");
                if (view.isPressed()) {
                    mViewModel.mLowSoundRequest.requestSoundFiled(0, 0);
    ...
    public void setSoundFiled(int x, int y) {
        ...
        isChange=false;
        mHandler.sendEmptyMessageDelayed(FROMUSER_TOUCH,5000);mViewModel.mLowSoundRequest.requestSoundFiled(x, y);
    }
