 ...
 */
public class LineCalibrationConditionSignalView extends BaseConditionSignalView {

    private WeakReference<AvmApaControlBehavior> wr;


    public LineCalibrationConditionSignalView(AvmApaControlBehavior behavior) {
        super();
    ...
    protected boolean shouldNotifySignalDataChanged(String key, int value) {
        if (null == wr) {
            Log.e("AdayoCamera", TAG + " - shouldNotifySignalDataChanged: failed because WeakReference<AvmControlBehavior> is null , return false");
            return false;
        }
        if (null == wr.get()) {
            Log.e("AdayoCamera", TAG + " - shouldNotifySignalDataChanged: failed because wr.get() is null , return false");
            return false;
        }
        ...
        return AvmApaControlBehavior.CALIBRATION_LINE == calibrationType;
    }
}
