 ...
 */
public class HvacSeatHeatingView extends BaseHvacImageView {
    public HvacSeatHeatingView(Context context) {
        super(context);
    }

    public HvacSeatHeatingView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public HvacSeatHeatingView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void initOther() {
        ...
        switch (areaID){
            ...
            case AREA_SEAT_PASSENGER:
                setVisibility(ConfigUtils.getKey(FRONT_SEAT_HEATING) == 1 ? View.VISIBLE : View.GONE);
                break;
            case AREA_SEAT_REAR_LEFT:
            case AREA_SEAT_REAR_RIGHT:
                setVisibility(ConfigUtils.getKey(SECOND_SEAT_HEATING) == 1 ? View.VISIBLE : View.GONE);
                break;
        }
    }
    ...
    protected void updateView(int value) {
        ...
        switch (status) {
            case HVAC_SEAT_HEATING_CLOSE:
                AAOP_HSkin.with(this).addViewAttrs(AAOP_HSkin.ATTR_SRC, R.drawable.icon_heating_close).applySkin(false);
                break;
            case HVAC_SEAT_HEATING_ONE:
                AAOP_HSkin.with(this).addViewAttrs(AAOP_HSkin.ATTR_SRC, R.drawable.icon_heating_one).applySkin(false);
                break;
            case HVAC_SEAT_HEATING_TWO:
                AAOP_HSkin.with(this).addViewAttrs(AAOP_HSkin.ATTR_SRC, R.drawable.icon_heating_two).applySkin(false);
                break;
            case HVAC_SEAT_HEATING_THREE:
                AAOP_HSkin.with(this).addViewAttrs(AAOP_HSkin.ATTR_SRC, R.drawable.icon_heating_thr).applySkin(false);
                break;
            default:
                break;
        }
