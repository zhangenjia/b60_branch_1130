 ...
 *三方通话-来电中
 */
public class ThirdWayInCommingState implements ICallState {

    private static final String TAG = ThirdWayInCommingState.class.getSimpleName();
    ...
    public ThirdWayInCommingState(List<View> visibleList, List<TextView> textList){
    ...
    public void updateDisplayStatus() {
        ...
        mCallAnswerSwitch.setVisibility(View.GONE);
        mShowDialingKeyboard.setVisibility(View.GONE);
        mThirdCallRL.setVisibility(View.VISIBLE);
        mCallInfoRL.setVisibility(View.GONE);
        mThirdCallInfoRL.setVisibility(View.VISIBLE);
        int drawable;
        if(mSendSMSMgr.needSendSMS()){
            drawable = R.drawable.selector_call_phone_hangup_sendsms;
        }else{
            drawable = R.drawable.selector_call_phone_hangup;
        }
         AAOP_HSkin
                 .with(mHangUp)
